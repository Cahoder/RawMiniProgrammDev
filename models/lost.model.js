...
var options = {
    ...
    collection : 'lost',   //在MongDB中默认使用Model的名字作为集合的名字，如过需要自定义集合的名字，可以通过设置这个选项
...
var LostSchema = new Schema({
    ...
    title:{type:String},//丢失的物品的名字
    content:{type:String},//丢失的物品的描述
    goods_type:{type:String},//丢失的物品的类型 一个数字代表一种类型
    telephone:{type:String},//联系电话
    position:{type:String},//丢失的位置
...
var Lost = mongoose.model('Lost', LostSchema);//创建模型
var Promise = require('bluebird');
Promise.promisifyAll(Lost);
Promise.promisifyAll(Lost.prototype);

module.exports = Lost;//建立模块，令全局可以访问这个模块